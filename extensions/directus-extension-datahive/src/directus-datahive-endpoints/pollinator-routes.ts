import { relay } from '../../../../datahive-core/src/orchestrator'
import { capitalizeFirstLetter } from '../utils'

const moduleName = "pollinator"
...
export default function (router: any) {
  ...
  router.get(`/${moduleName}/start/:projectId`, async (req: any, res: any) => {
    ...
    try {
      await relay(moduleName, "start", projectId);
  ...
  router.get(`/${moduleName}/pause/:runId`, (req: any, res: any) => {
    ...
    try {
      ...
      res.status(500).json({
        error: true,
        message: `Error pausing ${capModuleName} run: ${error.message || error}`
      });;
    }
  });
  ...
  router.get(`/${moduleName}/resume/:runId`, (req: any, res: any) => {
    ...
    try {
      ...
      res.status(500).json({
        error: true,
        message: `Error resuming ${capModuleName} run: ${error.message || error}`
      });
    }
  });
  ...
  router.get(`/${moduleName}/finish/:runId`, (req: any, res: any) => {
    ...
    try {
      ...
      res.status(500).json({
        error: true,
        message: `Error finishing ${capModuleName} run: ${error.message || error}`
      });
    }
  });


  router.get(`/${moduleName}/info/runs/:projectId`, async (req: any, res: any) => {
    const projectId = req.params.projectId;
    try {
      console.log(`Getting info for ${capModuleName} runs for project ID ${projectId}...`);
      res.send(`Info for ${capModuleName} runs retrieved successfully.`);
    } catch (error: any) {
      res.status(500).json({
        error: true,
        message: `Error getting infos for ${capModuleName} runs: ${error.message || error}`
      });
    }
  });
  ...
  router.get(`/${moduleName}/info/run/:runId`, async (req: any, res: any) => {
    ...
    try {
      ...
      res.status(500).json({
        error: true,
        message: `Error getting infos for ${capModuleName} run: ${error.message || error}`
      });
    }
  });
